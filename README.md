...
**演示脚本及运行日志见 `example.sh` 和 `example.log`**
...
- 当前 PDF/图片目录扫描为**非递归**（只扫描目录第一层）。
- 论文切块是按字符滑窗切分，未记录页码；如需页码级定位，需要在切块时保留页号与偏移映射。
- “自动分类”目前基于**向量库近邻主题投票**，当库样本较少时可能不稳定；可扩展为本地/云端 LLM 的主题分类器。
- Embedding 与向量库已抽象封装，可替换为本地模型（SentenceTransformers/CLIP）与其他向量库实现。